"""Core simulation engine for Lab 15.

Extends Lab 14 (Client-Side Calculations) with three new mechanics:

1. Pressure defense — factions spend spice to undo pressure on their own nodes.
   Spice delivered to an owned node with active pressure is consumed at rate
   `pressure_defense_rate` (1 spice removes that many accumulated pressure points).
   Remainder is added to stock as normal.

2. Pressure decay — accumulated pressure decays each tick when not continuously applied,
   at rate `pressure_decay_rate` (fraction removed per tick).

3. Local mutation rate — each node's effective mutation probability is scaled by a
   local multiplier that combines four signals:
     · high local spice concentration (stock / effective cap)
     · devastation level
     · incoming pressure (normalized by flip_threshold)
     · number of distinct foreign-faction neighbors
   Weights are individually configurable.
"""
...
def _decay_pressure(state: SimState) -> None:
    """Decay accumulated pressure on all nodes by `pressure_decay_rate` each tick."""
    rate = state.config.pressure_decay_rate
    if rate <= 0.0:
        return
    factor = 1.0 - rate
    for node in state.nodes.values():
        if node.pressure_accumulated <= 0.0:
            continue
        node.pressure_accumulated = max(0.0, node.pressure_accumulated * factor)
        node.pressure = {
            fid: v * factor
            for fid, v in node.pressure.items()
            if v * factor > 1e-3
        }
        if node.pressure_accumulated < 1e-3:
            node.pressure_accumulated = 0.0
            node.pressure = {}


def _update_devastation(state: SimState, flipped_ids: list[str]) -> None:
    """Update flip-based devastation after conquest resolution.

    Pressure-based and defense-based devastation increments are applied during
    _execute_transport (Lab 15). This function handles only the flip/recovery layer:
    ...
    for faction in state.active_factions():
        ...
        for src_id, dst_id, amount in faction.move_orders:
            ...
            if dst_node.owner_id == faction.faction_id:
                to_stock = delivered
                defense_rate = state.config.pressure_defense_rate
                if dst_node.pressure_accumulated > 0.0 and defense_rate > 0.0:
                    max_reducible = delivered * defense_rate
                    actual_reduction = min(dst_node.pressure_accumulated, max_reducible)
                    spice_spent = actual_reduction / defense_rate
                    if actual_reduction > 0.0:
                        old_total = dst_node.pressure_accumulated
                        dst_node.pressure_accumulated -= actual_reduction
                        remaining_ratio = dst_node.pressure_accumulated / old_total
                        dst_node.pressure = {
                            fid: v * remaining_ratio
                            for fid, v in dst_node.pressure.items()
                        }
                        to_stock = delivered - spice_spent
                        dst_node.devastation = min(
                            1.0,
                            dst_node.devastation + state.config.devastation_defense_rate * spice_spent,
                        )
                        _record_faction_action(
                            state,
                            faction.faction_id,
                            dst_id,
                            "defense",
                            f"spent {spice_spent:.1f} spice, reduced pressure on {dst_id} by {actual_reduction:.1f}",
                            resource_effects=[
                                ResourceEffect(
                                    resource_id=dst_node.spice_resource_id,
                                    effect_type="consume",
                                    quantity=float(spice_spent),
                                    source_id=dst_node.node_actor_id,
                                )
                            ],
                        )
                        state.event_log.append(
                            f"[tick {state.tick}] {faction.faction_id} spent {spice_spent:.1f} "
                            f"spice to reduce pressure on {dst_id} by {actual_reduction:.1f}"
                        )
                dst_node.spice_stock += max(0.0, to_stock)
                ...
                _record_faction_action(
                    ...
                    dst_id,
                    "transfer",
                    f"moved {to_stock:.1f} spice from {src_id} to {dst_id}",
                    resource_effects=[
                        ResourceEffect(
                            resource_id=src_node.spice_resource_id,
                            effect_type="transfer",
                            quantity=float(to_stock),
                ...
                dst_node.devastation = min(
                    1.0,
                    dst_node.devastation + state.config.devastation_pressure_rate * delivered,
                )
...
def _local_mutation_multiplier(state: SimState, node: Node, faction_id: str) -> float:
    """Return a ≥1 multiplier that scales the base mutation probability for this node.

    Four independent signals each add to the baseline of 1.0:
      1. spice concentration  — node.spice_stock / effective_cap
      2. devastation          — node.devastation
      3. pressure             — pressure_accumulated / flip_threshold
      4. neighboring foreign factions — distinct non-owner faction count among neighbors
    """
    cfg = state.config
    mul = 1.0

    stock_cap = max(1.0, _node_effective_stock_cap(state, node))
    spice_factor = _clamp01(node.spice_stock / stock_cap)
    mul += cfg.mutation_spice_weight * spice_factor

    mul += cfg.mutation_devastation_weight * node.devastation

    pressure_factor = _clamp01(node.pressure_accumulated / max(1.0, cfg.flip_threshold))
    mul += cfg.mutation_pressure_weight * pressure_factor

    foreign_fids: set[str] = set()
    for nb_id in state.neighbors_of(node.node_id):
        nb_owner = state.nodes[nb_id].owner_id
        if nb_owner is not None and nb_owner != faction_id:
            foreign_fids.add(nb_owner)
    n_others = max(1, len(state.active_factions()) - 1)
    faction_factor = _clamp01(len(foreign_fids) / n_others)
    mul += cfg.mutation_border_weight * faction_factor
...
def _mutate_and_check_secession(
    ...
    for faction in list(state.active_factions()):
        ...
        for nid in owned:
            ...
            local_mul = _local_mutation_multiplier(state, node, faction.faction_id)
            p_mutate = min(1.0, state.config.mutation_rate * (depth + 1) * local_mul)
...
def step(state: SimState) -> None:
    ...
    6.  Execute transport (defense spending happens here for owned-node deliveries)
    7.  Apply conquest → returns flipped list
    8.  Decay remaining pressure (Lab 15)
    9.  Update devastation (increment flipped, decrement stable)
    10. Apply stock cap (devastation-adjusted effective cap)
    11. Grow relations
    12. Mutate genomes with local multiplier (Lab 15) / check secession
    13. Check victory
    14. Increment tick
    """
    if state.game_over:
    ...
    _decay_pressure(state)             # Lab 15: decay unconsumed pressure
...
def create_sim(config: SimConfig) -> SimState:
    ...
    for node in state.nodes.values():
        ...
        _sync_node_stock_to_resource(state, node)
    state.event_log.append(
        f"[tick 0] Simulation started (Lab 15 — Enhanced Local Mechanics). "
...
def serialize_state(state: SimState) -> dict:
    """Return a minimal raw-state payload.

    Derived display values (node color, effective_stock_cap, effective_production,
    flip_count_in_window, faction total_spice / spice_income / node_count,
    perceived_relations) are intentionally omitted here.  The browser client
    computes them from the raw fields, keeping each /api/state response cheap
        ...
        nodes_out.append(
            {
                ...
                "flip_tick_history": list(node.flip_tick_history),
                "mutation_local_mul": round(
                    _local_mutation_multiplier(state, node, node.owner_id or ""), 3
                ),
            }
        )
