...
const factionMeta  = document.getElementById("faction-meta");
...
function renderFactions(state) {
  ...
  if (factionMeta) {
    factionMeta.textContent = sortedFactions.length > 0
      ? `${sortedFactions.length} active`
      : "no active factions";
  }
  ...
  for (const f of sortedFactions) {
    ...
    const capitalId = f.capital_id;
    const capitalNode = (state.nodes || []).find(n => n.node_id === capitalId);
    const ownsCapital = capitalNode && capitalNode.owner_id === f.faction_id;
    ...
    card.innerHTML = `
      <div class="faction-card-header">
        ...
        <span class="faction-name">${f.faction_id}${ownsCapital ? " ★" : ""}</span>
      </div>
