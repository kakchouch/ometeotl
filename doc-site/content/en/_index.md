...
- **Networks foundations layer** in `src/ometeotl_foundations/networks/` — pure-Python, adapter-agnostic first-order graph-theory specialization including a graph vocabulary (`GraphKind`, `GraphSpec`, predefined singletons `UNDIRECTED_SIMPLE`, `DIRECTED_SIMPLE`, `UNDIRECTED_WEIGHTED`, `DIRECTED_WEIGHTED`), structural protocols (`Graph`, `GraphBackend`), a pure-Python mutable graph primitive (`AdjacencyGraph`), a `SpaceRelationGraph`-backed read-only adapter (`SpaceRelationGraphAdapter`), generic typed network containers (`NetworkSpace[G]`, `NetworkExtent`, `NetworkMap`), a mutable ready-to-use class (`AdjacencyNetworkSpace`) whose topology IS a `SpaceRelationGraph` — the single source of truth — and `derive_space_relations_from_network()` — the bridge that derives a `SpaceRelationGraph` from any `Graph`-Protocol-compatible network.
- Complete class-reference documentation with code examples for all public classes across `ometeotl_core`, `ometeotl_foundations/spatial/`, and `ometeotl_foundations/networks/`.
- Current automated baseline: `757` collected tests.
