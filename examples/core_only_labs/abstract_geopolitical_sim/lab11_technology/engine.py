"""Core simulation engine for Lab 11 — Technology.

Extends Lab 10 (complex behaviour + relations + logistics + globalization) with
a per-faction **technology investment pipeline**.

Each faction carries a TechVector (Diplo, Cohé, Logi) ∈ [0, 1] that grows
monotonically via a 7-step stochastic pipeline (v0 → v6) running every tick:
  v0  die-roll — positive-octant unit vector, seeded by config.seed via state._rng
  v1  E-inertia — blend with rolling history; high E → more conservative
  v2  C-burst/cooldown — high C amplifies low-magnitude bursts, zeroes high ones
...
Tech effects (committed at tick T+1 start, one-tick delay):
  Diplo → inflates attacker's perceived relation with the Diplo holder, reducing
           retaliation.  High-Diplo factions can be aggressive while being attacked less.
  Cohé  → raises the Hamming secession threshold (cultural stability of territory).
  Logi  → reduces the proportional transport gas fee for this faction's shipments.

Logistics layer (inherited from Lab 3+):
  Spice stock lives in Node, not in Faction.
  Links have max_flow capacity; shipments pay a flat base cost plus a proportional fee.
  Conquest pressure comes from spice routed to enemy/neutral nodes.
...
def _random_unit_sphere_3d(rng: random.Random) -> tuple[float, float, float]:
    """Sample from the positive octant of the unit sphere using the provided RNG.

    Uses Gaussian projection so all directions in the positive octant are equally
    likely.  The RNG must be state._rng to keep v0 under config seed control.
    """
    while True:
        x = rng.gauss(0.0, 1.0)
        y = rng.gauss(0.0, 1.0)
...
@dataclass
class TechVector:
    """Per-faction technology levels on three independent axes (all in [0, 1])."""
...
class Faction:
    ...
    # Number of direct children spawned via secession (used to generate child IDs)
    children_spawned: int = 0
    # Effective Hamming threshold used in the most recent secession check (audit field)
    effective_hamming_threshold: float = 0.0
    # Technology state (Lab 11)
    tech: TechVector = field(default_factory=TechVector)
    tech_pending: TechVector = field(default_factory=TechVector)
    tech_investment_history: list[TechVector] = field(default_factory=list)
    tech_alpha: tuple[float, float, float] = (0.0, 0.0, 0.0)
...
def _get_perceived_relation(
    ...
    """Return the relation value as perceived by *observer_id* toward *target_id*.

    perceived = true_relation + diplo_bias_strength * max(0, diplo(target) - diplo(observer))

    Semantics:
    - If target has higher diplo tech, observer overestimates the relation quality.
    - Zero gap → zero bias → perceived == true.
    - Result is clamped to [0, 1].
    - The true relation graph is NEVER mutated.
    - The computation is performed by the engine; the observer's planning code only
      reads this return value and never accesses the target faction object directly.
    """
...
def _relation_pressure_factor(
    ...
    """Return offense multiplier from *perceived* relation level.

    High perceived relation → lower aggression.  Low → higher aggression.
    Perception is already biased by the diplo gap via _get_perceived_relation;
    this function does not access the defender faction object directly.
    """
...
def _execute_transport(state: SimState) -> None:
    ...
    for faction in state.active_factions():
        # Logi tech reduces the proportional gas fee for this faction
        ...
        for src_id, dst_id, amount in faction.move_orders:
            ...
            # Apply proportional gas fee (reduced by Logi tech)
...
def _apply_stock_cap(state: SimState) -> None:
    """Clamp each owned node's spice stock to the faction's logi-based cap.

    cap(node) = base_stock_cap + faction.tech.logi * logi_stock_cap_bonus

    Excess is destroyed in place — it is NOT redistributed to any other node.
    Called at end of tick, after all spice flows and conquest.
    """
    cfg = state.config
...
def _visible_neighbor_faction_ids(
    ...
    """Return the set of other active faction IDs visible to *faction*.

    In limited-perception mode, only factions that own a visible node count.
    In full mode (perception=None), all other active factions count.
    """
    ...
    if perception is not None:
        return {
            ...
            and state.nodes[nid].owner_id is not None
            and state.nodes[nid].owner_id != own_id
        }
    return {
        fid
...
def _compute_hamming_threshold(
    ...
    """Compute the effective secession Hamming threshold for *faction*.

    threshold = drift_threshold_bits
                + cohe_tech * cohe_hamming_bonus
                - max(0, mean_neighbor_cohe - cohe_tech) * cohe_hamming_decay
    Floored at min_hamming_threshold.

    Neighbor cohé is read from perception (limited mode) or all active factions (full mode).
    Isolated factions (no visible neighbors) receive zero decay.
    """
    cfg = state.config
...
def _mutate_and_check_secession(
    ...
    If any node's Hamming distance from its faction genome exceeds the dynamic
    threshold (see _compute_hamming_threshold), that node secedes.

    *perceptions* maps faction_id → Perception (or None for full-mode).
    When omitted, full-mode perception is assumed for all factions.
    ...
    for faction in list(state.active_factions()):
        ...
        # Compute and store the dynamic threshold for audit serialisation.
...
def _secede(state: SimState, node_id: str, parent_faction: Faction) -> str:
    ...
    # Safety: avoid collision if two secessions race to the same id
...
def _extract_tech_signals(
    ...
    """Read faction perception and produce the signal dict for the pipeline."""
    faction_id = faction.faction_id
    ...
    mean_relation = (
        sum(relations.values()) / len(relations) if relations else 0.5
    )
...
def _compute_tech_alpha(
    signals: dict,
    config: SimConfig,
) -> tuple[float, float, float]:
    """Map signals to normalised α (bias-direction vector for step 4)."""
...
def _compute_tech_investment_debug(
    ...
    """Run the 7-step investment pipeline; return (v6, v5) for invariant testing.

    PIPELINE INVARIANT: reads only from faction.behavior, faction.genome,
    faction.tech, faction.tech_investment_history, faction.move_orders,
    faction.admin_orders.  The only side-effect is writing faction.tech_alpha.
    No step reads from the output (v6 / tech_pending) of the same tick.
    """
    cfg = state.config
    behavior = faction.behavior  # snapshot — pipeline must NOT modify this

    # --- Step 1: Base vector (die roll) — sole stochastic step; uses state._rng ---
    v0 = _random_unit_sphere_3d(state._rng)

    # --- Step 2: E — investment trigger ---
    ...
    # Low E → always invest; high E → requires strong signal
    ...
    # --- Step 3: C — intensity profile ---
    C = behavior.concentration
    history = faction.tech_investment_history  # previous ticks only (invariant 3)
    last_mag = (
        _magnitude_3d(history[-1].diplo, history[-1].cohe, history[-1].logi)
        if history else 0.0
    )
    burst_threshold = 0.3 + C * 0.4  # [0.3, 0.7]
    # Post-burst cooldown if last tick was a burst; otherwise amplify
    intensity = max(0.0, 1.0 - C) if last_mag > burst_threshold else (1.0 + C)
    v2 = (v1[0] * intensity, v1[1] * intensity, v1[2] * intensity)

    # --- Step 4: O — directional bias ---
    ...
    if v2_mag > 1e-12:
        # Hadamard(α, bias): per-axis signal-weighted directional deformation
        deformation = (alpha[0] * bias[0], alpha[1] * bias[1], alpha[2] * bias[2])
        v3_raw = (v2[0] + deformation[0], v2[1] + deformation[1], v2[2] + deformation[2])
        v3_raw_mag = _magnitude_3d(*v3_raw)
        if v3_raw_mag > 1e-12:
            # Direction deformed; magnitude preserved from step 3
            v3 = (
    ...
    # --- Step 6: Z — historical inertia (previous ticks only) ---
    ...
    planned_spend = sum(
        amt + base_cost
        for _, _, amt in faction.move_orders + faction.admin_orders
    )
...
def _compute_tech_investment(
    ...
    """Run the investment pipeline and return v6 as a TechVector."""
...
def _compute_rubberband_multipliers(state: SimState, faction: Faction) -> TechVector:
    """Per-axis cost multipliers: leader penalty × follower acceleration."""
    cfg = state.config
    ...
    for axis in ("diplo", "cohe", "logi"):
        ...
        max_level = max(
            (getattr(f.tech, axis) for f in all_active), default=0.0
        )
        # Leader cost increase
        leader_mult = (
            1.0 + cfg.tech_leader_cost_multiplier
            if max_level > 1e-9 and faction_level >= max_level - 1e-9
            else 1.0
        )
        # Neighbor acceleration: cheaper per axis the further behind a neighbour
        neighbor_mult = 1.0
...
def _apply_tech_effects(state: SimState) -> None:
    """Commit last tick's pending investment to tech levels; update history buffer.

    Called at the very start of each tick (before any planning) so effects
    computed in tick T take hold in tick T+1 exactly.
    """
    ...
    for faction in state.factions.values():
        ...
        if pending.diplo > 0.0 or pending.cohe > 0.0 or pending.logi > 0.0:
            ...
            if len(faction.tech_investment_history) > window:
                faction.tech_investment_history = (
                    faction.tech_investment_history[-window:]
                )
...
def step(state: SimState) -> None:
    ...
    2.  Apply tech effects from previous tick (commit pending → tech levels)
    3.  Collect spice income into nodes
    4.  Apply rare globalization event (link growth or bridge creation)
    5.  For each faction: symbolic deliberation, plan moves, plan centralization,
        then run tech investment pipeline (after offense/defense/admin allocation)
    6.  Execute transport: move spice, apply base cost + proportional gas fee
    7.  Apply conquest (check flips, seize unallocated spice)
    8.  Mutate genomes and check secession
    ...
    _apply_tech_effects(state)  # effects from last tick become active now
    _collect_income(state)
    _apply_globalization(state)

    # Collect per-faction perceptions once so planning and secession use the
    # same tick-start visibility (before conquest changes ownership).
    ...
    for faction in state.active_factions():
        ...
        # Tech pipeline runs after offense/defense/admin so available spice is known
        faction.tech_pending = _compute_tech_investment(state, faction, perception)

    _execute_transport(state)
    _apply_conquest(state)
    _apply_stock_cap(state)              # Change 1: destroy excess after all flows
    _grow_relations(state)
    _mutate_and_check_secession(state, perceptions)   # Change 2: perception-aware threshold
...
def create_sim(config: SimConfig) -> SimState:
    ...
    world = World(id="lab11-technology-world")
    relation_graph = SpaceRelationGraph()
    ...
    state.event_log.append(
        f"[tick 0] Simulation started (Lab 11 - Technology). "
...
def serialize_state(state: SimState) -> dict:
    ...
    for fid, faction in state.factions.items():
        ...
        factions_out[fid] = {
            ...
            # Audit: Change 2 — effective Hamming threshold used last tick
            "effective_hamming_threshold": round(faction.effective_hamming_threshold, 3),
            # Audit: Change 3 — perceived relation values (biased by diplo gap)
            "perceived_relations": {
                other_fid: round(_get_perceived_relation(state, fid, other_fid), 3)
                for other_fid in state.factions
                if other_fid != fid
                and state.relation_between(fid, other_fid) is not None
            },
        }
        ...
        nodes_out.append(
            {
                ...
                # Audit: Change 1 — effective stock cap for this node
                "stock_cap": round(
                    (
                        state.config.base_stock_cap
                        + state.factions[node.owner_id].tech.logi
                          * state.config.logi_stock_cap_bonus
                    )
                    if node.owner_id and node.owner_id in state.factions
                    else state.config.base_stock_cap,
                    2,
                ),
            }
        )
