...
class SimConfig:
    ...
    drift_threshold_fraction:
        Fraction of genome bits that may differ before a node becomes
        autonomous under a vassal relation (Lab 6).
        E.g. 0.5 on a 16-bit genome means ≥ 8 bits different triggers
        autonomy/vassalization checks.
    top_secession_threshold_fraction:
        Fraction of genome bits that may differ from the top ancestor genome
        before full independent secession (outside vassal hierarchy).
        Must be >= drift_threshold_fraction.
    genome_length:
    ...
    vassal_tribute_fraction:
        Fraction of vassal capital stock transferred to parent capital each tick.
        Models loyalty and economic extraction in the hierarchy.
    """
    ...
    top_secession_threshold_fraction: float = 0.78
    ...
    vassal_tribute_fraction: float = 0.12
    ...
    def drift_threshold_bits(self) -> int:
        """Minimum Hamming distance (bits) that triggers vassal autonomy."""
        return max(1, round(self.drift_threshold_fraction * self.genome_length))

    @property
    def top_secession_threshold_bits(self) -> int:
        """Minimum Hamming distance (bits) to top ancestor for full secession."""
        return max(1, round(self.top_secession_threshold_fraction * self.genome_length))
    ...
    def validate(self) -> None:
        ...
        if not 0.0 < self.top_secession_threshold_fraction <= 1.0:
            raise ValueError("top_secession_threshold_fraction must be in (0, 1]")
        if self.top_secession_threshold_fraction < self.drift_threshold_fraction:
            raise ValueError(
                "top_secession_threshold_fraction must be >= drift_threshold_fraction"
            )
        ...
        if not 0.0 <= self.vassal_tribute_fraction <= 1.0:
            raise ValueError("vassal_tribute_fraction must be in [0, 1]")
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "top_secession_threshold_fraction": self.top_secession_threshold_fraction,
            ...
            "vassal_tribute_fraction": self.vassal_tribute_fraction,
            "drift_threshold_bits": self.drift_threshold_bits,
            "top_secession_threshold_bits": self.top_secession_threshold_bits,
        }
