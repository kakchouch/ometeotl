...
class SimConfig:
    ...
    # ---- Change 1: Logistics stock cap ----
    base_stock_cap: float = 50.0
    # Maximum spice a node with logi_tech = 0 can hold. Excess is destroyed each tick.
    logi_stock_cap_bonus: float = 100.0
    # Additional cap granted at logi_tech = 1. cap(node) = base + logi * bonus.

    # ---- Change 2: Cohesion Hamming threshold decay ----
    cohe_hamming_bonus: float = 5.0
    # Absolute bits added to drift threshold at cohe_tech = 1.
    cohe_hamming_decay: float = 4.0
    # Bits subtracted per unit of mean-neighbor cohé gap above own cohé.
    min_hamming_threshold: float = 1.0
    # Floor: effective threshold cannot drop below this regardless of decay.

    # ---- Change 3: Diplomacy gap bias ----
    diplo_bias_strength: float = 0.3
    # Coefficient for the diplo gap term:
    # perceived_rel(B→A) = true_rel + diplo_bias_strength * max(0, diplo(A) − diplo(B))
