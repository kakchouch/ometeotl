"""Core simulation engine for Lab 14 — Client-Side Calculations.
...
def create_sim(config: SimConfig) -> SimState:
    ...
    world = World(id="lab14-client-side-calculations-world")
    relation_graph = SpaceRelationGraph()
    ...
    for node in state.nodes.values():
        ...
        _sync_node_stock_to_resource(state, node)
    state.event_log.append(
        f"[tick 0] Simulation started (Lab 14 - Client-Side Calculations). "
