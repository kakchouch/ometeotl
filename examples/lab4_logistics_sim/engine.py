"""Core simulation engine for Lab 4 — Logistics Multi-Agent Simulation.
...
def _plan_moves(
    ...
    Strategy:
    1. Score offensive targets (unowned neighbours) by spice_flow / distance.
    2. Score defensive targets (owned nodes under pressure) by
       pressure_accumulated * (1 + spice_flow).
    3. Each owned source node with surplus stock routes toward the
       highest-scoring reachable target via the best adjacent hop,
       subject to link remaining capacity and max_spice_move_fraction.
    """
    ...
    for nid in offense_targets:
        ...
        scores[nid] = state.nodes[nid].spice_flow / d
    ...
    best_target = max(scores, key=lambda nid: scores[nid])
    base_cost = state.config.transport_base_cost

    for src_id in sorted(owned):
        ...
        max_dispatch = (src_node.spice_stock - base_cost) * state.config.max_spice_move_fraction

        next_hop = _next_hop_toward(state, src_id, best_target, owned, faction_id)
        if next_hop is None:
            continue
...
def create_sim(config: SimConfig) -> SimState:
    ...
    world = World(id="lab4-logistics-sim-world")
    relation_graph = SpaceRelationGraph()
    ...
    state.event_log.append(
        f"[tick 0] Simulation started (Lab 4 - Logistics). "
