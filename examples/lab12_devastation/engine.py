"""Core simulation engine for Lab 12 — Devastation.
...
class Node:
    ...
    flip_tick_history: list[int] = field(default_factory=list)
...
def _node_effective_production(
    ...
    """
    devas = devastation_override if devastation_override is not None else node.devastation
    base = config.base_node_production * (1.0 - devas * config.devastation_production_penalty)
...
def _update_devastation(state: SimState, flipped_ids: list[str]) -> None:
    ...
    for nid, node in state.nodes.items():
        ...
        node.flip_tick_history = [
            t for t in node.flip_tick_history
            if state.tick - t < cfg.devastation_window_size
        ]
        if nid in flipped_set:
            node.devastation = min(1.0, node.devastation + cfg.devastation_flip_increment)
            node.flip_tick_history.append(state.tick)
        else:
            node.devastation = max(0.0, node.devastation - cfg.devastation_recovery_rate)
...
def _build_goal_and_strategy(
    ...
    if pressure > state.config.flip_threshold * 0.20:
        mode = "stabilize"; priority = 0.9
    elif disconnected_ratio > 0.15:
        mode = "connect"; priority = 0.8
    elif node_share < 0.34 and mean_relation < 0.55:
        mode = "expand"; priority = 0.85
    elif mean_relation > 0.75:
        mode = "reconcile"; priority = 0.7
    else:
        mode = "balanced"; priority = 0.65

    if behavior.engagement_threshold > 0.65 and mode in ("expand", "connect"):
        mode = "stabilize"; priority = 0.8
        ...
        mode = "reconcile"; priority = 0.75
    elif behavior.objective_bias > 0.7 and mode in ("stabilize", "reconcile"):
        if pressure < state.config.flip_threshold * 0.10:
            mode = "expand"; priority = 0.80
    elif behavior.centralization < 0.3 and mode == "stabilize":
        mode = "balanced"; priority = 0.65

    goal = Goal(
        id=f"goal-{fid}-t{tick}", actor_id=fid, kind="final", priority=priority,
        status="active",
        target_condition={
            "mode": mode, "node_share": round(node_share, 3),
            "mean_relation": round(mean_relation, 3), "mean_pressure": round(pressure, 3),
    ...
    strategy = Strategy(
        id=f"strategy-{fid}-t{tick}", actor_id=fid, goal_id=goal.id,
        root_node_id=root_node.node_id, nodes=[root_node],
    ...
    if mode == "stabilize":
        base_e, base_o, base_c, base_l, base_z = (
            behavior.engagement_threshold, behavior.objective_bias,
            behavior.concentration, behavior.liquidity_preference, behavior.centralization,
        )
        alignment = (1 - base_e) + (1 - base_o) + (1 - base_c) + base_l + base_z
        scale_factor = 1.0 - (alignment / 5.0 * 0.5)
        return SymbolicIntent(goal=goal, strategy=strategy, mode=mode,
            engagement_shift=-0.08*scale_factor, objective_shift=-0.28*scale_factor,
            concentration_shift=-0.10*scale_factor, liquidity_shift=0.08*scale_factor,
    ...
    if mode == "connect":
        ...
        return SymbolicIntent(goal=goal, strategy=strategy, mode=mode,
            engagement_shift=-0.02*scale_factor, objective_shift=-0.10*scale_factor,
            concentration_shift=-0.20*scale_factor, liquidity_shift=-0.04*scale_factor,
    ...
    if mode == "expand":
        ...
        scale_factor = 1.0 - (alignment / 5.0 * 0.5)
        return SymbolicIntent(goal=goal, strategy=strategy, mode=mode,
            engagement_shift=0.12*scale_factor, objective_shift=0.30*scale_factor,
            concentration_shift=0.16*scale_factor, liquidity_shift=-0.10*scale_factor,
            centralization_shift=-0.10*scale_factor)
    if mode == "reconcile":
        base_e, base_o, base_c, base_l, base_z = (
            behavior.engagement_threshold, behavior.objective_bias,
        ...
        scale_factor = 1.0 - (alignment / 4.5 * 0.4)
        return SymbolicIntent(goal=goal, strategy=strategy, mode=mode,
            engagement_shift=-0.10*scale_factor, objective_shift=-0.16*scale_factor,
            concentration_shift=-0.08*scale_factor, liquidity_shift=0.10*scale_factor,
            centralization_shift=0.06*scale_factor)
    return SymbolicIntent(goal=goal, strategy=strategy, mode="balanced",
        engagement_shift=0.0, objective_shift=0.0, concentration_shift=0.0,
        liquidity_shift=0.0, centralization_shift=0.0)
...
def _plan_moves(
    ...
    for nid in offense_targets:
        ...
        perceived_prod = _node_effective_production(state.nodes[nid], cfg, perceived_devas)
...
def _execute_transport(state: SimState) -> None:
    ...
    for faction in state.active_factions():
        effective_gas_fee = max(
            0.0, gas_fee * (1.0 - faction.tech.logi * state.config.tech_logi_cost_reduction)
        )
...
def _collect_income(state: SimState) -> None:
    ...
    for nid, node in state.nodes.items():
        if node.owner_id is not None:
            node.spice_stock += _node_effective_production(node, cfg)
...
def _visible_neighbor_faction_ids(
    ...
    return {fid for fid, f in state.factions.items() if fid != own_id and not f.is_eliminated}
...
def _secede(state: SimState, node_id: str, parent_faction: Faction) -> str:
    ...
    new_faction = Faction(
        faction_id=new_fid, capital_id=node_id, genome=new_genome,
        behavior=_genome_to_ecloz(new_genome), color=_genome_to_color(new_genome),
        tech=TechVector(diplo=parent_faction.tech.diplo, cohe=parent_faction.tech.cohe,
                        logi=parent_faction.tech.logi),
        tech_pending=TechVector(diplo=parent_faction.tech_pending.diplo,
...
def _check_victory(state: SimState) -> None:
    ...
    if state.config.max_ticks > 0 and state.tick >= state.config.max_ticks:
        best = max(state.active_factions(),
...
def _add_runtime_link(state: SimState, a: str, b: str, capacity: float) -> bool:
    ...
    rel = SpaceRelation(source_space_id=key[0], target_space_id=key[1],
    ...
    if perception is not None:
        ...
        for nid in owned:
            if bfs.get(nid, -1) < 0:
                disconnected_owned += 1
    return {
        "known_nodes": known_nodes, "owned_fraction": owned_fraction,
        "relations": relations, "mean_relation": mean_relation,
        "min_relation": min_relation, "disconnected_owned": disconnected_owned,
        "total_pressure_received": total_pressure, "mean_pressure_received": mean_pressure,
        "known_ratio": known_ratio,
    }


def _compute_tech_alpha(signals: dict, config: SimConfig) -> tuple[float, float, float]:
    pressure_norm = _clamp01(signals["total_pressure_received"] / max(1.0, config.flip_threshold))
...
def _compute_tech_investment_debug(
    ...
    pressure_sig = _clamp01(signals["total_pressure_received"] / max(1.0, cfg.flip_threshold))
    ...
    if v2_mag > 1e-12:
        ...
        if v3_raw_mag > 1e-12:
            v3 = (v3_raw[0]/v3_raw_mag*v2_mag, v3_raw[1]/v3_raw_mag*v2_mag,
    ...
    spend_rate = _clamp01(1.0 - L * _clamp01(total_spice / max(cfg.tech_reserve_reference, 1e-9)))
    ...
    if history:
        ...
        v5 = (Z*hist_mean[0]+(1.0-Z)*v4[0], Z*hist_mean[1]+(1.0-Z)*v4[1],
              Z*hist_mean[2]+(1.0-Z)*v4[2])
    else:
        v5 = v4
    base_cost = cfg.transport_base_cost
    planned_spend = sum(amt+base_cost for _, _, amt in faction.move_orders+faction.admin_orders)
    ...
    for faction in state.factions.values():
        ...
        if pending.diplo > 0.0 or pending.cohe > 0.0 or pending.logi > 0.0:
            ...
            if len(faction.tech_investment_history) > window:
                faction.tech_investment_history = faction.tech_investment_history[-window:]
...
def create_sim(config: SimConfig) -> SimState:
    ...
    world = World(id="lab12-devastation-world")
    relation_graph = SpaceRelationGraph()
    ...
    for i, cap_id in enumerate(capital_ids):
        ...
        faction = Faction(
            faction_id=fid, capital_id=cap_id, genome=genome,
            behavior=_genome_to_ecloz(genome), color=_genome_to_color(genome),
        )
    ...
    for raw_node in raw.nodes:
        ...
        nodes[nid] = Node(
            node_id=nid, spice_flow=raw_node.spice_flow,
            x=raw_node.x, y=raw_node.y, owner_id=owner, genome=genome,
            spice_stock=config.initial_node_spice,
        )

    state = SimState(
        config=config, world=world, relation_graph=relation_graph,
        nodes=nodes, factions=factions, links=links,
    )
    state._rng = random.Random(rng.randint(0, 2**32 - 1))
    state.event_log.append(
        f"[tick 0] Simulation started (Lab 12 - Devastation). "
...
def serialize_state(state: SimState) -> dict:
    ...
    for fid, faction in state.factions.items():
        ...
        factions_out[fid] = {
            ...
            "symbolic": (
                ...
                if faction.symbolic_intent is not None else None
            ),
            "tech": {
    ...
    for nid, node in state.nodes.items():
        ...
        flip_count = sum(
            1 for t in node.flip_tick_history
