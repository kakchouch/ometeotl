"""Minimal HTTP server for the Lab 15 — Enhanced Local Mechanics simulation web UI.
...
_MAX_HISTORY = 40
_AUTORUN_HISTORY_STRIDE = 4
_history: deque[SimState] = deque(maxlen=_MAX_HISTORY)
...
PORT = 8778  # Dedicated Lab 15 port
    ...
    while _autorun_running:
        with _state_lock:
            if SIM_STATE.game_over:
                _autorun_running = False
                break
            # Keep step-back useful without deep-copying full state every tick.
            if _AUTORUN_HISTORY_STRIDE <= 1 or SIM_STATE.tick % _AUTORUN_HISTORY_STRIDE == 0:
...
def run_server(port: int = PORT) -> None:
    ...
    print(f"Lab 15 — Enhanced Local Mechanics server running at http://127.0.0.1:{port}/")
