/**
 * app.js — Lab 6 Vassal Hierarchy Simulation client
 *
 * Responsibilities:
...
const API = "http://127.0.0.1:8770";  // Lab 6 server port
...
function renderFactions(state) {
  ...
  for (const f of sortedFactions) {
    ...
    const isTop = !f.parent_faction_id;
    const headerTag = isTop ? "▲" : "•";
    const lineage = f.parent_faction_id
      ? `Parent: ${f.parent_faction_id} | Top: ${f.top_ancestor_id}`
      : `Top hierarchy: ${f.top_ancestor_id}`;
    card.innerHTML = `
      <div class="faction-card-header">
        <div class="faction-color-dot" style="background:${f.color}"></div>
        <span class="faction-name">${headerTag} ${f.faction_id}</span>
      </div>
      ...
      <div class="faction-stats">
        <span>Depth: ${f.hierarchy_depth ?? 0}</span>
        <span>${lineage}</span>
      </div>
      <div class="faction-stats">
