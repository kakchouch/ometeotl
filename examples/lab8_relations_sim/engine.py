"""Core simulation engine for Lab 8 — Relations Simulation.
...
def _apply_centralization(state: SimState, faction: Faction, node: Node) -> int:
    ...
    affordable = len(drift_indices) if admin_cost <= 0 else int(spendable_spice // admin_cost)
...
def _relation_pressure_factor(state: SimState, attacker_id: str, defender_id: str) -> float:
...
def create_sim(config: SimConfig) -> SimState:
    ...
    state.event_log.append(
        f"[tick 0] Simulation started (Lab 8 - Relations). "
