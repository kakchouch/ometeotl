...
class SimConfig:
    ...
    devastation_pressure_rate: float = 0.01
    # Devastation added per unit of pressure delivered to a node (attacker side).
    devastation_defense_rate: float = 0.01
    # Devastation added per unit of spice spent on defense (defender side).
    ...
    # ---- Lab 15: Pressure Defense & Local Mutation Rate ----
    pressure_defense_rate: float = 1.5
    # spice-to-pressure exchange rate: 1 delivered spice removes this much accumulated pressure
    pressure_decay_rate: float = 0.1
    # fraction of remaining pressure removed per tick when not continuously applied; 0 = no decay
    mutation_spice_weight: float = 0.5
    # how much high local spice concentration (stock/cap) boosts local mutation rate
    mutation_devastation_weight: float = 0.5
    # how much devastation boosts local mutation rate
    mutation_pressure_weight: float = 0.5
    # how much incoming pressure (normalized by flip_threshold) boosts local mutation rate
    mutation_border_weight: float = 0.5
    # how much neighboring foreign factions boost local mutation rate
    ...
    def validate(self) -> None:
        ...
        if self.devastation_pressure_rate < 0.0:
            raise ValueError("devastation_pressure_rate must be >= 0")
        if self.devastation_defense_rate < 0.0:
            raise ValueError("devastation_defense_rate must be >= 0")
        ...
        # Lab 15
        if self.pressure_defense_rate < 0.0:
            raise ValueError("pressure_defense_rate must be >= 0")
        if not 0.0 <= self.pressure_decay_rate <= 1.0:
            raise ValueError("pressure_decay_rate must be in [0, 1]")
        if self.mutation_spice_weight < 0.0:
            raise ValueError("mutation_spice_weight must be >= 0")
        if self.mutation_devastation_weight < 0.0:
            raise ValueError("mutation_devastation_weight must be >= 0")
        if self.mutation_pressure_weight < 0.0:
            raise ValueError("mutation_pressure_weight must be >= 0")
        if self.mutation_border_weight < 0.0:
            raise ValueError("mutation_border_weight must be >= 0")
    ...
    def to_dict(self) -> dict:
        d = {
            ...
            "devastation_pressure_rate": self.devastation_pressure_rate,
            "devastation_defense_rate": self.devastation_defense_rate,
            ...
            # Lab 15
            "pressure_defense_rate": self.pressure_defense_rate,
            "pressure_decay_rate": self.pressure_decay_rate,
            "mutation_spice_weight": self.mutation_spice_weight,
            "mutation_devastation_weight": self.mutation_devastation_weight,
            "mutation_pressure_weight": self.mutation_pressure_weight,
            "mutation_border_weight": self.mutation_border_weight,
        }
        return d
