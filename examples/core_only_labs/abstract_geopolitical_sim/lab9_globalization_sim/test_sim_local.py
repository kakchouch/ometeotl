"""Tests for Lab 9 — Globalization multi-agent simulation.

These tests live in examples and are NOT part of the tracked tests/ tree.
Run with:
    python -m pytest -q examples/lab9_globalization_sim/test_sim_local.py
"""
...
from examples.core_only_labs.abstract_geopolitical_sim.lab9_globalization_sim.config import SimConfig
from examples.core_only_labs.abstract_geopolitical_sim.lab9_globalization_sim.graph_gen import build_graph, bfs_distances
from examples.core_only_labs.abstract_geopolitical_sim.lab9_globalization_sim.engine import (
    SimState,
    Node,
    Link,
    Faction,
...
from examples.core_only_labs.abstract_geopolitical_sim.lab9_globalization_sim.perception import (
...
def test_geographic_can_start_disconnected_when_enabled():
    ...
    dists = bfs_distances(g, "node-0")
    unreachable = [nid for nid, d in dists.items() if d < 0]
    assert unreachable, "Expected at least one disconnected node in Lab 9 geographic mode"
...
def test_node_flip_seizes_spice():
    ...
    from examples.core_only_labs.abstract_geopolitical_sim.lab9_globalization_sim.engine import _apply_conquest
