/**
 * app.js — Lab 13 Ometeotl Retro-Fit Simulation client
 *
 * Responsibilities:
...
const API = "http://127.0.0.1:8776";  // Lab 13 server port
...
// ── Auto-untangle animation ──────────────────────────────────── //
let untangleRAF  = null; // requestAnimationFrame ID (null = stopped)
let untangleIter = 0;
const UNTANGLE_MAX_ITERS      = 500;
const UNTANGLE_STEPS_PER_FRAME = 5;
...
// ── Incremental force step (reads/writes nodeOverrides) ─────── //
// Returns max node displacement (px) — used for convergence detection.
function computeForceStep(nodes, edges, temperature) {
  if (nodes.length < 2) return 0;

  const nodeR = nodes.length >= 30 ? 14 : nodes.length >= 22 ? 17 : NODE_R;
  const k     = Math.sqrt((SVG_W * SVG_H) / nodes.length) * 0.85;
  const MARGIN = nodeR + 22;

  // Load from nodeOverrides (falling back to server positions)
  const pos = {};
  for (const n of nodes) {
    const ov = nodeOverrides[n.node_id];
    pos[n.node_id] = ov
  ...
  // 1. Node-node repulsion with hard-overlap boost
  const minSep = nodeR * 2.4;
  for (let i = 0; i < ids.length; i++) {
    for (let j = i + 1; j < ids.length; j++) {
      const a = ids[i], b = ids[j];
      const dx = pos[b].x - pos[a].x;
      const dy = pos[b].y - pos[a].y;
      const dist = Math.hypot(dx, dy) || 0.5;
      // Boost repulsion strongly when nodes overlap
      const overlap = dist < minSep ? 1 + (minSep - dist) / minSep * 5 : 1;
      const f = k * k / dist * overlap;
      const ux = dx / dist * f, uy = dy / dist * f;
      if (!pinnedNodes.has(b)) { fx[b] += ux; fy[b] += uy; }
      if (!pinnedNodes.has(a)) { fx[a] -= ux; fy[a] -= uy; }
    }
  }

  // 2. Edge spring attraction (target length ≈ k)
  for (const e of edges) {
    const ap = pos[e.a], bp = pos[e.b];
    if (!ap || !bp) continue;
    const dx = bp.x - ap.x;
    const dy = bp.y - ap.y;
    const dist = Math.hypot(dx, dy) || 0.5;
    const f = (dist - k) / dist;
    if (!pinnedNodes.has(e.b)) { fx[e.b] -= dx * f; fy[e.b] -= dy * f; }
  ...
  for (const id of ids) {
    ...
    const mag = Math.hypot(fx[id], fy[id]) || 1e-6;
    const step = Math.min(maxStep, mag) / mag;
    const dispX = fx[id] * step;
    const dispY = fy[id] * step;
    pos[id].x = Math.max(MARGIN, Math.min(SVG_W - MARGIN, pos[id].x + dispX));
    pos[id].y = Math.max(MARGIN, Math.min(SVG_H - MARGIN, pos[id].y + dispY));
    maxDisp = Math.max(maxDisp, Math.hypot(dispX, dispY));
  }

  // Persist back to nodeOverrides
  for (const n of nodes) {
    if (!pinnedNodes.has(n.node_id)) {
      nodeOverrides[n.node_id] = { x: pos[n.node_id].x / SVG_W, y: pos[n.node_id].y / SVG_H };
    }
...
function stopAutoUntangle() {
  if (untangleRAF !== null) {
    cancelAnimationFrame(untangleRAF);
    untangleRAF = null;
  }
  const btn = document.getElementById("btn-auto-untangle");
  if (btn) { btn.textContent = "⚡ Auto-untangle"; btn.classList.remove("running"); }
}

function startAutoUntangle() {
  if (untangleRAF !== null) return;
  untangleIter = 0;
  const btn = document.getElementById("btn-auto-untangle");
  if (btn) { btn.textContent = "⏹ Stop"; btn.classList.add("running"); }

  function frame() {
    if (!lastState) { stopAutoUntangle(); return; }
    let maxDisp = 0;
    for (let i = 0; i < UNTANGLE_STEPS_PER_FRAME; i++) {
      // Temperature: starts hot (45 px), cools to 0.3 px
      const temp = Math.max(0.3, 45 * Math.pow(1 - untangleIter / UNTANGLE_MAX_ITERS, 1.3));
      const d = computeForceStep(lastState.nodes, lastState.edges, temp);
      maxDisp = Math.max(maxDisp, d);
      untangleIter++;
    }
    renderGraph(lastState);
    if (untangleIter >= UNTANGLE_MAX_ITERS || maxDisp < 0.2) {
      stopAutoUntangle();
      return;
    }
    untangleRAF = requestAnimationFrame(frame);
  }
  untangleRAF = requestAnimationFrame(frame);
}

function toggleAutoUntangle() {
  if (untangleRAF !== null) stopAutoUntangle();
  else startAutoUntangle();
}
...
document.getElementById("btn-auto-untangle").addEventListener("click", () => {
  toggleAutoUntangle();
});
