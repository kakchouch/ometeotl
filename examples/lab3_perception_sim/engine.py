"""Core simulation engine for the multi-agent graph simulation (Lab 3 - Perception Variant).
...
- Symbolic AI: priority scoring for border nodes (full or limited information based on perception_mode)
...
# --------------------------------------------------------------------------- #
# Symbolic AI — priority assignment (with perception support)                 #
# --------------------------------------------------------------------------- #


def _assign_priorities(state: SimState, faction: Faction, perception: Optional[Perception] = None) -> None:
    """Score each node (offensive + defensive) and allocate spice resources.

    Offensive targets (border nodes):
      In limited mode: only visible neighbors (from perception)
      In full mode: all unowned neighbors
      Score: node.spice_flow / (bfs_distance + 1)

    Defensive targets (owned nodes under threat):
      Nodes owned by the faction that have accumulated pressure from enemies
    ...
    # Collect offensive targets (perception-aware or full vision)
    if perception is not None:
        # Limited perception mode: only target visible neighbors
        offense_targets = visible_border_targets(perception, owned)
    else:
        # Full vision mode: target all border nodes
...
def step(state: SimState) -> None:
    ...
    2. Assign AI priorities (with optional perception-based limited vision)
    ...
    for faction in state.active_factions():
        # Compute faction perception if perception_mode is "limited"
        perception = None
...
def create_sim(config: SimConfig) -> SimState:
    ...
    world = World(id="lab3-perception-sim-world")
    relation_graph = SpaceRelationGraph()
    ...
    for a, b in raw.edges:
        ...
        # Also add to world.space_relation_graph so Sensor can perceive relations
    ...
    state.event_log.append(f"[tick 0] Simulation started (Lab 3 - Perception). {config.num_factions} factions, {config.num_nodes} nodes, perception_mode={config.perception_mode}.")
    return state
