"""Minimal HTTP server for the Spatial Map Lab web UI.

Endpoints:
  GET  /api/state    — full serialised SimState as JSON
  POST /api/step     — advance one tick
  POST /api/reset    — reinitialise (optional JSON config body)
...
def state_is_stopped(state: SimState) -> bool:
    """Return True when the simulation should auto-stop."""
    return state.config.max_ticks > 0 and state.tick >= state.config.max_ticks
...
def run_server(port: int = PORT) -> None:
    ...
    print(f"Spatial Map Lab server running at http://127.0.0.1:{port}/")
