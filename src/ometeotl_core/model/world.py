...
class World(Space):
    ...
    space_object_graph: SpaceObjectGraph = field(
        default_factory=SpaceObjectGraph
    )
    space_relation_graph: SpaceRelationGraph = field(
        default_factory=SpaceRelationGraph
    )
    model_registry: WorldModelRegistry = field(
        default_factory=WorldModelRegistry
    )
    _authority_mode_enabled: bool = field(
        default=False, init=False, repr=False
    )
    _authority_token: Optional[str] = field(
    ...
    def __post_init__(self) -> None:
        ...
        self.model_registry.set_mutation_guard(
            self._assert_mutation_allowed
        )
        self.set_mutation_guard(
            self._assert_bound_object_mutation_allowed
        )
    ...
    def enable_authority_mode(self, token: str) -> None:
        ...
        """
        _require_non_empty(
            token, "Authority token cannot be empty"
        )
        if self._authority_mode_enabled:
            raise RuntimeError(
                "Authority mode is already enabled for this world"
            )
    ...
    def add_space(
        self, space: Space, authority_token: Optional[str] = None
    ) -> None:
    ...
    def unregister_object(
        ...
        self._assert_mutation_allowed(authority_token)
        self.model_registry.unregister(
            obj_id, authority_token=authority_token
        )
    ...
    def to_dict(self) -> JsonMap:
        ...
        """
        base = super().to_dict()
        base["space_object_graph"] = (
            self.space_object_graph.to_dict()
        )
        base["space_relation_graph"] = (
            self.space_relation_graph.to_dict()
        )
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "World":
        ...
        base_kwargs = _base_kwargs_from_typed_payload(
            data, "world"
        )
        attributes = dict(base_kwargs.get("attributes") or {})
        is_root_world = bool(
            attributes.get("is_root_world", True)
        )
        return cls(
            **base_kwargs,
