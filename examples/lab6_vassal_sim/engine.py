...
def _execute_transport(state: SimState) -> None:
    ...
    for faction in state.active_factions():
        for src_id, dst_id, amount in faction.move_orders:
            ...
            if (
                dst_node.owner_id == faction.faction_id
                or state.are_allied(faction.faction_id, dst_node.owner_id)
            ):
...
def _register_actor_for_faction(state: SimState, faction: Faction, actor_label: str) -> None:
    try:
