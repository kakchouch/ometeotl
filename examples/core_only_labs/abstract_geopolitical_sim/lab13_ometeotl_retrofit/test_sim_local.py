"""Tests for Lab 13 — Ometeotl Retro-Fit.

Invariant tests are written before implementation, as required by the spec.

Run with:
    python -m pytest -q examples/lab13_ometeotl_retrofit/test_sim_local.py
"""
...
from examples.core_only_labs.abstract_geopolitical_sim.lab13_ometeotl_retrofit.config import SimConfig
from examples.core_only_labs.abstract_geopolitical_sim.lab13_ometeotl_retrofit.engine import (
    SimState,
    Node,
    Faction,
    BehaviorProfile,
...
from examples.core_only_labs.abstract_geopolitical_sim.lab13_ometeotl_retrofit.perception import get_faction_perception
...
def test_serialize_includes_devastation_audit_fields():
    """Node serialization must include devastation, effective_stock_cap, effective_production,
    and flip_count_in_window."""
    ...
    for node_data in d["nodes"]:
        ...
        assert "effective_stock_cap" in node_data, f"Missing 'effective_stock_cap' for {nid}"
        assert "effective_production" in node_data, f"Missing 'effective_production' for {nid}"
        assert "flip_count_in_window" in node_data, f"Missing 'flip_count_in_window' for {nid}"
