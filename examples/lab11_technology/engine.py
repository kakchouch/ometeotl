...
def _relation_pressure_factor(
    ...
    Diplo tech of the defender inflates the attacker's perceived relation,
    reducing the attacker's incentive to press pressure on the defender.
    This creates an asymmetry: high-Diplo factions can be aggressive while
    suffering less retaliation.
    """
    rel = state.relation_between(attacker_id, defender_id)
    if rel is None:
        return 1.0
    # Diplo inflates perceived relation from the attacker's perspective.
    if defender_id in state.factions:
        diplo = state.factions[defender_id].tech.diplo
        effect = state.config.tech_diplo_perception_effect
        rel = _clamp01(rel + diplo * effect)
...
def _mutate_and_check_secession(state: SimState) -> list[str]:
    ...
    for faction in list(state.active_factions()):
        ...
        for nid in owned:
            ...
            # Check secession — Cohé tech raises the Hamming threshold
            hamming = _hamming_distance(node.genome, faction.genome)
            cohe_bonus = round(
                faction.tech.cohe
                * state.config.genome_length
                * state.config.tech_cohe_threshold_bonus
            )
            effective_drift_threshold = state.config.drift_threshold_bits + cohe_bonus
            if hamming >= effective_drift_threshold:
