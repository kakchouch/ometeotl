"""Core simulation engine for Lab 7 — Centralization Simulation.
...
def _drift_behavior(
    ...
    return BehaviorProfile(
        ...
        centralization=_clamp01(
            behavior.centralization + rng.uniform(-mag, mag)
        ),
    )
...
def _apply_centralization(state: SimState, faction: Faction, node: Node) -> int:
    ...
    if z <= 0.0 or node.spice_stock <= 0.0:
        return 0
    ...
    affordable = len(drift_indices) if admin_cost <= 0 else int(node.spice_stock // admin_cost)
...
def create_sim(config: SimConfig) -> SimState:
    ...
    world = World(id="lab7-centralization-sim-world")
    relation_graph = SpaceRelationGraph()
    ...
    state.event_log.append(
        f"[tick 0] Simulation started (Lab 7 - Centralization). "
